...
export class EloService {
  ...
  private computeNewRatings(homeRating: number, awayRating: number, homeScore: number, awayScore: number) {
  ...
  async initializeFromHistory(): Promise<{ teamsProcessed: number; matchesReplayed: number }> {
    ...
    const getRating = (team: string) => {
      if (!ratings.has(team)) ratings.set(team, { rating: DEFAULT_RATING, matches: 0 });
    ...
    for (const m of matches) {
      ...
      const updated = this.computeNewRatings(home.rating, away.rating, m.home_score, m.away_score);
    ...
    const { error } = await this.supabase.client.from('team_ratings').upsert(rows, { onConflict: 'team_name' });
    if (error) throw error;
  ...
  async getRating(teamName: string): Promise<number> {
    const { data } = await this.supabase.client.from('team_ratings').select('rating').eq('team_name', teamName).maybeSingle();
